...
SELECT
    ot.consecutivo_ot,
    ot.fecha_elaboracion_ot,
    TRUNC(SYSDATE - ot.fecha_elaboracion_ot) AS dias_pendiente,
    c.nombre_cli || ' ' || c.apellido_1_cli AS cliente,
    c.telefono_cli,
    m.placa_mot,
...
-- Vista 2: Solo productos activos
...
-- Vista 3: Productos con sus impuestos
...
CREATE OR REPLACE FORCE NONEDITIONABLE VIEW "US_PPI"."VW_PRODUCTOS_CON_IMPUESTOS" ("COD_PRO", "NOMBRE_PRO", "DESCRIPCION_PRO", "PRECIO_PRO", "STOCK_PRO", "STOCK_PRO_MIN", "ESTADO_PRODUCTO", "COD_PRO_IMP", "NOMBRE_IMP", "PORCENTAJE_PRO_IMP", "VALOR_IMPUESTO", "PRECIO_CON_IMPUESTO") AS
  SELECT
    p.cod_pro,
    p.nombre_pro,
    p.descripcion_pro,
...
-- Comentarios sobre las vistas de productos
COMMENT ON TABLE vw_productos IS 'Vista de productos con informacion del estado';
COMMENT ON TABLE vw_productos_activos IS 'Vista de productos activos unicamente (cod_est_pro = 1)';
COMMENT ON TABLE vw_productos_impuestos IS 'Vista de relacion productos-impuestos con detalles del impuesto';
...
-- Vista 2: Clientes con cantidad de motos
...
-- Vista 2: Motos con información de marca
...
-- Vista 3: Motos con información completa del cliente
...
-- Vista 2: Marcas con cantidad de motos
...
-- Vista 3: Marcas con detalle de motos registradas
...
-- ============================================
-- VISTAS PARA reclamos
-- ============================================
-- Comentarios reclamos
...
SELECT
    ...
    CASE
        WHEN ot.fecha_fin_garantia_ot >= TRUNC(SYSDATE) THEN 'VIGENTE'
        WHEN ot.fecha_fin_garantia_ot < TRUNC(SYSDATE) THEN 'VENCIDA'
        ELSE 'SIN INFORMACIÓN'
    END AS estado_garantia,
    CASE
        WHEN ot.fecha_fin_garantia_ot >= TRUNC(SYSDATE)
        THEN TRUNC(ot.fecha_fin_garantia_ot - SYSDATE)
        ELSE NULL
...
COMMENT ON TABLE vw_clientes IS 'Vista básica de clientes con nombre completo concatenado';
COMMENT ON TABLE vw_clientes_resumen IS 'Vista de clientes con el total de motos que poseen';

-- Comentarios motos
COMMENT ON TABLE vw_motos IS 'Vista básica de motos sin información relacionada';
COMMENT ON TABLE vw_motos_marcas IS 'Vista de motos con información de la marca';
COMMENT ON TABLE vw_motos_detalle IS 'Vista de motos con información completa de marca y cliente';

-- Comentarios marcas
COMMENT ON TABLE vw_marcas IS 'Vista básica de marcas';
COMMENT ON TABLE vw_marcas_resumen IS 'Vista de marcas con el total de motos registradas';
COMMENT ON TABLE vw_marcas_motos IS 'Vista de marcas con detalle de todas sus motos';
